...
public class RolesController {
    ...
    public JsonVO update(@RequestBody Role role) {
        try {
    ...
    public JsonVO create(@RequestBody Role role) {
        try {
            roleService.create(role);
            return new JsonVO(true, "创建角色成功");
        } catch (Exception e) {
            e.printStackTrace();
            return new JsonVO(false, "发生未知错误");
        }
    ...
    public JsonVO updateRolesPermissions(@RequestBody Map<String, Object> dataMap) {
        try {
    ...
    public List<TreeVO> getZTreeForRolePermissionAll(@RequestParam("id") Long id) {
        try {
            ...
            treeList.add(new TreeVO(role.getId(), role.getDescription(), true, role.getPid()));

            //遍历设置子节点数据
            for (Permission permission : dataList) {
                treeList.add(new TreeVO(permission.getId(), permission.getDescription(), false, permission.getRid()));
            }
            return treeList;
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public List<TreeVO> getZTreeForAllRoles() {
        try {
            List<TreeVO> treeList = new ArrayList<TreeVO>();
